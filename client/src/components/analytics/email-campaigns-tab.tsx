...
export default function EmailCampaignsTab({
  ...
  const handleTimeRangeChangeInternal = (value: string) => {
  ...
  const filteredCampaigns = useMemo(() => {
    if (!emailCampaignsData || !(emailCampaignsData as any)?.campaigns) {
      return [];
    }

    return (emailCampaignsData as any).campaigns.filter((campaign: any) => {
      const dateToCheck =
  ...
  const filteredDailyData = useMemo(() => {
    if (!emailStatisticsData || !(emailStatisticsData as any)?.dailyData) {
      return [];
    }

    return (emailStatisticsData as any).dailyData.filter((item: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <EmailCampaignsChart
        ...
        summary={(emailStatisticsData as any)?.summary}
        isLoading={isLoadingEmailStatistics}
        timeRange={timeRange}
      />
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        <Select
          ...
          onValueChange={handleTimeRangeChangeInternal}
        >
